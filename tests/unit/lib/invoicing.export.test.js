...
test('invoicing exports round every figure to three decimals', async (t) => {
  const { out } = await runExport(
    ...
    { buckets: 1, hashrateMhs: 123456789012.3, poolHashrateHs: 1.234567890123e17 }
  )

  t.is(out.split('\n')[1], '"01/08/2026","00:00","444.444","98.765","123.457","123.457"', 'matches the precision the UI exports')
  t.pass()
})
...
test('invoicing hashes period labels stay UTC in a west-of-UTC display timezone', async (t) => {
  const { out } = await runExport(
    'invoicing-hourly-hashes',
    { start: START, end: START + HOUR_MS, timezone: 'America/Sao_Paulo', format: 'csv' },
    { buckets: 1 }
  )

  t.is(out.split('\n')[1], '"01/08/2026","00:00","356.4","80","100","99"', 'the UTC bucket is not relabeled to Jul 31 21:00')
  t.pass()
})
